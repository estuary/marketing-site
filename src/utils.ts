...
export const normalizeConnector = (connector: any) => {
  ...
  const type = connector.connectorTagsByConnectorIdList?.[0]?.protocol as 'capture' | 'materialization';
  return {
    id: connector.id,
    ...
    slug: regex_result ? `/${type === 'capture' ? 'source' : 'destination'}/${regex_result[2]}` : null,
  };
};
